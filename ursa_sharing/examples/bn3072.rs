...
impl Field<Bn3072> for Bn3072 {
    ...
    fn mul_assign(&mut self, rhs: &Bn3072) {
        let n = BigNum::from_dec_str(MODULUS).unwrap();
        let r = BigNum::from_slice(self.0.to_vec().as_slice()).unwrap();
        let mut ctx = BigNumContext::new().unwrap();
    ...
    fn div_assign(&mut self, rhs: &Bn3072) {
        let n = BigNum::from_dec_str(MODULUS).unwrap();
        let r = BigNum::from_slice(self.0.to_vec().as_slice()).unwrap();
...
impl Field<Bn3072, Bgn3072> for Bgn3072 {
    type FieldSize = U384;
    ...
    fn one() -> Self {
        // Base generator for commitments
        Self(BigNum::from_u32(2).unwrap())
    }
    ...
    fn from_bytes<B: AsRef<[u8]>>(value: B) -> SharingResult<Self> {
        Ok(Self(BigNum::from_slice(value.as_ref()).unwrap()))
    }
...
fn main() {
    ...
    combine_single::<Bn3072, Bgn3072>();
    println!("Combine combinations success");
    combine_all_combinations::<Bn3072, Bgn3072>();
}
