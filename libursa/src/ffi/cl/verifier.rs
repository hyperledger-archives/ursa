...
#[no_mangle]
pub extern fn ursa_cl_verifier_new_proof_verifier(proof_verifier_p: *mut *const c_void) -> ErrorCode {
    trace!("ursa_cl_verifier_new_proof_verifier: >>> {:?}", proof_verifier_p);

    check_useful_c_ptr!(proof_verifier_p, ErrorCode::CommonInvalidParam1);

    let res = match Verifier::new_proof_verifier() {
        Ok(proof_verifier) => {
            trace!("ursa_cl_verifier_new_proof_verifier: proof_verifier: {:?}", proof_verifier);
            unsafe {
                *proof_verifier_p = Box::into_raw(Box::new(proof_verifier)) as *const c_void;
                trace!("ursa_cl_verifier_new_proof_verifier: *proof_verifier_p: {:?}", *proof_verifier_p);
            }
            ErrorCode::Success
        }
...
#[no_mangle]
pub extern fn ursa_cl_proof_verifier_add_sub_proof_request(proof_verifier: *const c_void,
    ...
    trace!("ursa_cl_proof_verifier_add_sub_proof_request: >>> proof_verifier: {:?}, \
                                                                     sub_proof_request: {:?} ,\
                                                                     credential_schema: {:?}, \
...
#[no_mangle]
pub extern fn ursa_cl_proof_verifier_verify(proof_verifier: *const c_void,
                                                   proof: *const c_void,
                                                   nonce: *const c_void,
...
mod tests {
    ...
    fn ursa_cl_verifier_new_proof_verifier_works() {
        ...
        _add_sub_proof_request(proof_verifier_p, credential_schema, non_credential_schema, credential_pub_key, sub_proof_request, ptr::null(), ptr::null());
    ...
    fn ursa_cl_proof_verifier_add_sub_proof_request_works() {
        ...
        let err_code = ursa_cl_proof_verifier_add_sub_proof_request(proof_verifier,
                                                                           sub_proof_request,
                                                                           credential_schema,
                                                                           non_credential_schema,
                                                                           credential_pub_key,
    ...
    fn ursa_cl_proof_verifier_verify_works_for_primary_proof() {
        ...
        let mut valid = false;
        let err_code = ursa_cl_proof_verifier_verify(proof_verifier, proof, proof_building_nonce, &mut valid);
        assert_eq!(err_code, ErrorCode::Success);
        assert!(valid);
