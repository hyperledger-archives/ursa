...
impl RevocationRegistryDelta {
    pub fn fromParts(rev_reg_from: &JsValue,
                     rev_reg_to: &RevocationRegistry,
                     issued: &JsValue,
                     revoked: &JsValue) -> Result<RevocationRegistryDelta, JsValue> {
        ...
        let rrd = cl::RevocationRegistryDelta::from_parts(rev_reg_from.as_ref(), &rev_reg_to.0, &issued, &revoked);
...
#[wasm_bindgen]
impl Witness {
    pub fn new(rev_idx: u32,
               max_cred_num: u32,
               issuance_by_default: bool,
               rev_reg_delta: &RevocationRegistryDelta,
               rev_tails_accessor: &SimpleTailsAccessor) -> Result<Witness, JsValue> {
        let w = maperr!(cl::Witness::new(rev_idx, max_cred_num, issuance_by_default, &rev_reg_delta.0, &rev_tails_accessor.0));
        Ok(Witness(w))
    }
    ...
    pub fn update(&mut self,
                  ...
                  max_cred_num: u32,
                  rev_reg_delta: &RevocationRegistryDelta,
                  rev_tails_accessor: &SimpleTailsAccessor) -> Result<(), JsValue> {
        maperr!(self.0.update(rev_idx, max_cred_num, &rev_reg_delta.0, &rev_tails_accessor.0));
        Ok(())
    }
}
...
impl ProofBuilder {
    ...
    pub fn addSubProofRequest(&mut self, sub_proof_request: SubProofRequest,
                                 ...
                                 rev_reg: &JsValue,
                                 witness: &JsValue) -> Result<(), JsValue> {
        ...
        maperr!(self.0.add_sub_proof_request(&spr, &cs, &ncs, &credential_signature.0, &cv, &credential_pub_key.0, rev_reg.as_ref(), witness.as_ref()));
        Ok(())
    }
...
impl ProofVerifier {
    ...
    pub fn addSubProofRequest(&mut self,
                              sub_proof_request: SubProofRequest,
                              credential_schema: CredentialSchema,
                              non_credential_schema: NonCredentialSchema,
                              credential_pub_key: &CredentialPublicKey,
                              rev_key_pub: &JsValue,
                              rev_reg: &JsValue) -> Result<(), JsValue> {
        let spr = finalize!(sub_proof_request);
        let cs = finalize!(credential_schema);
        let ncs =  finalize!(non_credential_schema);
        ...
        maperr!(self.0.add_sub_proof_request(&spr, &cs, &ncs, &credential_pub_key.0, rev_key_pub.as_ref(), rev_reg.as_ref()));
        Ok(())
    }

    pub fn verify(&self,
                  proof: &Proof,
                  nonce: &Nonce) -> Result<bool, JsValue> {
...
#[allow(non_snake_case)]
impl Issuer {
    pub fn newCredentialDef(credential_schema: CredentialSchema,
                              non_credential_schema: NonCredentialSchema,
                              support_revocation: bool) -> Result<CredentialDefinition, JsValue> {

        let cs = finalize!(credential_schema);
        let ncs = finalize!(non_credential_schema);
        let (pk, sk, kp) = maperr!(cl::issuer::Issuer::new_credential_def(&cs, &ncs, support_revocation));
    ...
    pub fn signCredential(prover_id: &str,
                          ...
                          credential_values: CredentialValues,
                          credential_pub_key: &CredentialPublicKey,
                          credential_priv_key: &CredentialPrivateKey) -> Result<IssuedCredential, JsValue> {
        let cv = finalize!(credential_values);

        let (cs, scp) = maperr!(cl::issuer::Issuer::sign_credential(prover_id,
    ...
    pub fn signCredentialWithRevocation(prover_id: &str,
                                        blinded_credential_secrets: &BlindedCredentialSecrets,
                                        blinded_credential_secrets_correctness_proof: &BlindedCredentialSecretsCorrectnessProof,
                                        credential_nonce: &Nonce,
                                        credential_issuance_nonce: &Nonce,
                                        ...
                                        rev_tails_accessor: &SimpleTailsAccessor) -> Result<IssuedCredentialWithRevocation, JsValue> {
        let cv = finalize!(credential_values);

        let (cs,  scp, delta) = maperr!(cl::issuer::Issuer::sign_credential_with_revoc(prover_id,
            &blinded_credential_secrets.0,
            &blinded_credential_secrets_correctness_proof.0,
            &credential_nonce.0,
            &credential_issuance_nonce.0,
...
#[allow(non_snake_case)]
impl Prover {
    pub fn blindCredentialSecrets(credential_pub_key: &CredentialPublicKey,
                                  credential_key_correctness_proof: &CredentialKeyCorrectnessProof,
                                  credential_values: CredentialValues,
                                  credential_nonce: &Nonce) -> Result<ProverBlindedCredentialSecrets, JsValue> {
        let cv = finalize!(credential_values);
        let (blinded_credential_secrets,
             credential_secrets_blinding_factors,
             blinded_credential_secrets_correctness_proof) =
            maperr!(cl::prover::Prover::blind_credential_secrets(&credential_pub_key.0,
        ...
        Ok(ProverBlindedCredentialSecrets {
            ...
            credentialSecretsBlindingFactors: CredentialSecretsBlindingFactors(credential_secrets_blinding_factors),
            blindedCredentialSecretsCorrectnessProof: BlindedCredentialSecretsCorrectnessProof(blinded_credential_secrets_correctness_proof)
        })
    }
    pub fn processCredentialSignature(issued_credential: &IssuedCredential,
                                      credential_values: CredentialValues,
                                      ...
                                      rev_key_pub: &JsValue,
                                      rev_reg: &JsValue,
                                      witness: &JsValue) -> Result<CredentialSignature, JsValue> {
        let rev_key_pub = check_opt_reference!(rev_key_pub, RevocationPublicKey);
        let rev_reg = check_opt_reference!(rev_reg, RevocationRegistry);
        let witness = check_opt_reference!(witness, Witness);
        ...
        maperr!(cl::prover::Prover::process_credential_signature(&mut cs,
                                                         &cv,
                                                         &issued_credential.signatureCorrectnessProof.0,
                                                         &credential_secrets_blinding_factors.0,
                                                         &credential_pub_key.0,
                                                         &nonce.0,
                                                         rev_key_pub.as_ref(), rev_reg.as_ref(), witness.as_ref()));
